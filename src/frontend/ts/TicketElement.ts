...
export default class TicketElement {
	...
	static get markup() {
		...
		<div class="d-flex justify-content-start align-items-center">
			<input class="col-1 status" type="checkbox" disabled>
	...
	bindToDOM(
		...
		this.editBtnEl.addEventListener('click', this.onEditTicket.apply(
			this, [shortDescription, longDescription, status]
		));
		this.removeBtnEl.addEventListener('click', this.onRemoveTicket.bind(this));
	}

	onEditTicket(shortDescription: string, longDescription: string, status: boolean) {
		return () => {
			const modal = App.modals.editTicket;
			const inputId = modal.element.querySelector(ModalTicket.inputIdSelector) as HTMLInputElement;
			inputId.value = this.editBtnEl.dataset.id ?? '';
			modal.changeValueShortDescriptionEl(shortDescription);
			modal.changeValueLongDescription(longDescription);
			modal.changeValueStatus(status);
		};
	}
	...
	onRemoveTicket() {
		...
		App.widgets.toast.bindToCallbackForBtnOk(() => {
			toast.hide();
			Ticket.remove({id: this.removeBtnEl.dataset.id as string}, () => {
